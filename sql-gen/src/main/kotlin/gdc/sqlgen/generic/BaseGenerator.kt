                ...
                val baseCond = run {
                    ...
                    val comparisonValue = when (val v = e.value) {
                        ...
                        is ComparisonValue.ScalarValue -> when (v.value_type.uppercase()) {
                            ...
                            ScalarType.DATETIME.name -> DSL.cast(
                                DSL.inline(v.value),
                                SQLDataType.TIMESTAMP
                            )

                            ScalarType.DATE.name -> DSL.cast(
                                DSL.inline(v.value),
                                SQLDataType.DATE
                            )

                            ScalarType.TIME.name -> DSL.cast(
                                DSL.inline(v.value),
                                SQLDataType.TIME
                            )

                            ScalarType.TIME_WITH_TIMEZONE.name -> DSL.cast(
                                DSL.inline(v.value),
                                SQLDataType.TIMEWITHTIMEZONE
                            )

                            ScalarType.DATETIME_WITH_TIMEZONE.name -> DSL.cast(
                                DSL.inline(v.value),
